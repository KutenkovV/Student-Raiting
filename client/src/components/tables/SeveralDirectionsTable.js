...
const SeveralDirectionsTable = ({ data, itemsPerPage, startFrom, onChange }) => {
        ...
        <tbody>
          {slicedData.map((item) => (
            <tr key={item.id} className="loadTr">
              ...
              <td>{item.fullname}</td>
              <td>
                <div className={item.nid.destination ? "tdTrue" : "tdFalse"}>
                  {item.nid.point}
                </div>
              </td>
              <td>
                <div className={item.ud.destination ? "tdTrue" : "tdFalse"}>
                  {item.ud.point}
                </div>
              </td>
              <td>
                <div className={item.sd.destination ? "tdTrue" : "tdFalse"}>
                  {item.sd.point}
                </div>
              </td>
              <td>
                <div className={item.od.destination ? "tdTrue" : "tdFalse"}>
                  {item.od.point}
                </div>
              </td>
              <td>
                <div className={item.ktd.destination ? "tdTrue" : "tdFalse"}>
                  {item.ktd.point}
                </div>
              </td>
              ...
              <td
                onClick={() => {
                  ...
                  if (item.nid.destination === true) {
                    setDirections((Array) => [
                      ...Array,
                      "НАУЧНАЯ ДЕЯТЕЛЬНОСТЬ",
                    ]);
                  }
                  if (item.ud.destination === true) {
                    setDirections((Array) => [
                      ...Array,
                      "УЧЕБНАЯ ДЕЯТЕЛЬНОСТЬ",
                    ]);
                  }
                  if (item.sd.destination === true) {
                    setDirections((Array) => [
                      ...Array,
                      "СПОРТИВНАЯ ДЕЯТЕЛЬНОСТЬ",
                    ]);
                  }
                  if (item.od.destination === true) {
                    setDirections((Array) => [
                      ...Array,
                      "ОБЩЕСТВЕННАЯ ДЕЯТЕЛЬНОСТЬ",
                    ]);
                  }
                  if (item.ktd.destination === true) {
                    setDirections((Array) => [
                      ...Array,
