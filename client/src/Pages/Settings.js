...
const Settings = () => {
  ...
  useEffect(() => {
    items.map((item) => {
      if (item.course.title === 'НИД') { setnidValue(item.count) }
      if (item.course.title === 'КТД') { setktdValue(item.count) }
      if (item.course.title === 'УД') { setudValue(item.count) }
      if (item.course.title === 'СД') { setsdValue(item.count) }
      if (item.course.title === 'ОД') { setodValue(item.count) }
    })
  }, [items]);
        ...
        <div class="mainSettings row mt-4">
          <div class="col">
            <div class="col m-4">
              ...
              <input class="form-control" type="number" id="nidInput" onClick={() => setSaveActive(false)} value={nidValue} onChange={(event) => setnidValue(event.target.value)} />
            </div>
            <div class="col m-4">
              <label for="udInput" class="form-label">
                Учебная деятельность
              </label>
              <input class="form-control" type="number" id="udInput" onClick={() => setSaveActive(false)} value={udValue} onChange={(event) => setudValue(event.target.value)} />
            </div>
            <div class="col m-4">
              <label for="sdInput" class="form-label">
                Спортивная деятельность
              </label>
              <input class="form-control" type="number" id="sdInput" onClick={() => setSaveActive(false)} value={sdValue} onChange={(event) => setsdValue(event.target.value)} />
            </div>
          </div>
          <div class="col">
            <div class="col m-4">
              ...
              <input class="form-control" type="number" id="odInput" onClick={() => setSaveActive(false)} value={odValue} onChange={(event) => setodValue(event.target.value)} />
            </div>
            <div class="col m-4">
              <label for="ktdInput" class="form-label">
                Культурно-творческая деятельность
              </label>
              <input class="form-control" type="number" id="ktdInput" onClick={() => setSaveActive(false)} value={ktdValue} onChange={(event) => setktdValue(event.target.value)} />
            </div>
